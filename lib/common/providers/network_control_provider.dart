...
part 'network_control_provider.g.dart';
...
class NetworkController extends _$NetworkController {
  ...
  Future<NetworkInfo> build() async {
    ref.onDispose(() {
      print("<<<<<<< NetworkControllerProvider dispose <<<<<<<");
    });
    print(">>>>>>> NetworkControllerProvider build >>>>>>");
  ...
  Future<bool> connectNetwork({required String ssid, String? password, NetworkSecurity security = NetworkSecurity.WPA}) async {
    try {
      if(await Permission.location.request().isGranted) {
        final value = await WiFiForIoTPlugin.connect(ssid, password: password, security: security, joinOnce: true, withInternet: true);
        if(value) {
